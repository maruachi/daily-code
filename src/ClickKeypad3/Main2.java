...
public class Main2 {
    private static Map<Character, Side> keyFinder = createKeyFinder();
    private static Map<Character, Position> keyPosition = createKeyPosition();
    ...
    private static String solution(int[] numbers, String hand) {
        ...
        for(int number : numbers){
            keys.add(Character.forDigit(number, 10));
        }
        ...
        for(char key : keys){
            ...
            if(keySide == Side.MIDDLE){
                handSide = findHandSide(hand, rKey, lKey, key);
            }
    ...
    private static Side findHandSide(String hand, char rKey, char lKey, char key) {
        ...
        if(lDistance == rDistance){
            return Side.valueOf(hand.toUpperCase());
        }

        return rDistance < lDistance ? Side.RIGHT : Side.LEFT;
    }
